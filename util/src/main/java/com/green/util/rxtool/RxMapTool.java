...
import com.green.util.rxtool.model.Gps;
...
public class RxMapTool {
    ...
    public static void openMap(Context mContext, Gps gpsFrom, Gps gpsTo, String storeName) {
        ...
        if (RxPackageManagerTool.haveExistPackageName(mContext, RxConstants.GAODE_PACKAGE_NAME)) {
            ...
            //检测都未安装时，跳转网页版高德地图
        } else if (RxPackageManagerTool.haveExistPackageName(mContext, RxConstants.TENCENT_PACKAGE_NAME)) {
            RxMapTool.openTencentMapToGuide(mContext, gpsTo, storeName);
        } else {
     ...
     * @param storeName
     */
    private static void openTencentMapToGuide(Context mContext, Gps gpsTo, String storeName) {
        String uriString = null;
        Gps gps = RxLocationTool.GPS84ToGCJ02(gpsTo.getLongitude(), gpsTo.getLatitude());
        StringBuilder builder = new StringBuilder("qqmap://map/routeplan?type=drive&policy=0&referer=zhongshuo");
        builder.append("&to=").append(storeName)
                .append("&tocoord=").append(gpsTo.getLatitude())
                .append(",")
                .append(gpsTo.getLongitude());
        uriString = builder.toString();
        Intent intent = new Intent(Intent.ACTION_VIEW);
        intent.setPackage(RxConstants.TENCENT_PACKAGE_NAME);
