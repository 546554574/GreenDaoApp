package com.green.dao.helper.rxjavahelper;
...
public class RxResultHelper {
    ...
    private static final int RESPONSE_SUCCESS_CODE = 1; //大部分为200
                ...
                return tObservable.flatMap(
                        new Function<ResponseData<T>, Observable<T>>() {
                            ...
                            public Observable<T> apply(ResponseData<T> tResponseData) throws Exception {
                                //可以相应更改
                                if (tResponseData.getStatus() == RESPONSE_SUCCESS_CODE) {
                                    return Observable.just(tResponseData.getData());
                                } else if (tResponseData.getStatus() == RESPONSE_ERROR_CODE) {
                                    return Observable.error(new Exception(tResponseData.getInfo()));
                                } else {
